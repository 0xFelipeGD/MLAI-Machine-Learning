#!/usr/bin/env python3
"""
scratch_test.py — quick smoke check that the AGRO .tflite files load
and produce output on a dummy input.

Run from anywhere; the script locates models/ relative to its own
location (the project root, two levels up). Missing models are reported
but don't crash the run, so you can use this script incrementally as you
produce each model:
...
def main() -> int:
    ...
    if n_skip:
        print("(skipped files don't exist yet — see training/README.md)")
